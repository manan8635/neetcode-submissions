...
// here condition will be for INF , not -1 ,. because neigbour tho 0 bhi ho skta h na nd we don't want that neighbour
    ...
    void bfs(int m , int n , queue <state > q ,vector<vector<int>>& visited, vector<vector<int>>& grid ){
...
// is que me good point is distance hame grid[i][j] se hi maintain ho pa rha h 
// so hame koi distance array ya tuple maintain krne ki jrrut nhi h
